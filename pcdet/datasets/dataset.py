...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        # An overview
        # 1. __init()__:
        #   1.1. initialize the self.params
        #   1.2. create self.client for file reading
        #   1.3. create self.point_feature_encoder, self.data_augmentor, self.data_processor
        # 2. self.prepare_data():
        #   usually called by the __getitem__() of the child class
        #   2.1 forward self.data_augmentor if self.training:
        #       get 'gt_boxes_mask' (True if gt_names in self.class_names)
        #   2.2 filter out boxes by class names
        #       if has gt_boxes:
        #           filter out gt_boxes if their names not in self.class_names
        #       if has gt_boxes_2d:
        #           filter out gt_boxes_2d if their names not in self.class_names
        #   2.3 forward point_feature_encoder if has points
        #   2.4 forward data_processor
        #   2.5 if self.training and no gt_boxes:
        #       recursively call self.__getitem__() until gt_boxes is not None
        # 3. self.collate_batch():
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['points_prev', 'points', 'voxel_coords']:
                    coors = []
    ...
    def _generate_intervals(self, start_id, end_id, max_interval):
        # Two examples:
        # >> generate_intervsals(0, 6, 3)
        # >> [[0, 1], [0, 2], [0, 3], [1, 4], [2, 5], [3, 6]]
        # >> generate_intervals(0, 6, 2)
        # >> [[0, 1], [0, 2], [1, 3], [2, 4], [3, 5], [4, 6]]
        # Notice that, last_ix is exclusive
        intervals = []
        for last_ix in range(start_id+1, end_id+1):
            first_ix = max(start_id, last_ix - max_interval)
