...
def process_single_sequence(sequence_file, save_path, sampled_interval, client, has_label=True, use_two_returns=True):
    ...
    dataset = tf.data.TFRecordDataset(client._map_path(str(sequence_file)), compression_type='')
...
def convert_prv_frame_to_cur(pc_prv, pose_prv, pose_cur):
    expand_pc_prv = np.concatenate([pc_prv[:, :3], np.ones((pc_prv.shape[0], 1))], axis=-1)
    pc_prv_global = np.dot(expand_pc_prv, pose_prv.T)[:, :3]
    expand_pc_prv_global = np.concatenate([pc_prv_global, np.ones((pc_prv_global.shape[0], 1))], axis=-1)
    pc_prv2cur = np.dot(expand_pc_prv_global, np.linalg.inv(pose_cur.T))[:, :3]
    pc_prv2cur = np.concatenate([pc_prv2cur, pc_prv[:, 3:]], axis=-1)
    return pc_prv2cur
...
def test_convert_prv_frame_to_cur():
    from tools.visual_utils.open3d_vis_utils import draw_scenes_with_2pcs

    root_dir = "./data/waymo/"
    info_dir = os.path.join(root_dir, "waymo_processed_data_infos_train.pkl")
    info = pickle.load(open(info_dir, "rb"))

    seq_name = "segment-1208303279778032257_1360_000_1380_000_with_camera_labels"
    for idx_start in range(len(info)):
        if info[idx_start]["point_cloud"]["lidar_sequence"] == seq_name:
            break
    pc_idxs = [48, 98]
    ...
    poses = []
    for i in range(2):
        idx = idx_start + pc_idxs[i]
        pc_path = os.path.join(root_dir, "waymo_processed_data", seq_name, "%04d.npy" % pc_idxs[i])
        pcs.append(np.load(pc_path))

        poses.append(info[idx]["pose"].reshape((4, 4)))

    draw_scenes_with_2pcs(pcs[1], pcs[0])
    pc_prv2cur = convert_prv_frame_to_cur(pcs[0], poses[0], poses[1])
    draw_scenes_with_2pcs(pcs[1], pc_prv2cur)
