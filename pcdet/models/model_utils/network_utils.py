...
def make_fc_layers(fc_cfg, input_channels, output_channels=None, linear=True, norm_fn=None):
    ...
    for k in range(0, fc_cfg.__len__()):
        fc_layers.extend([
            ...
            # nn.GroupNorm(num_groups=32, num_channels=fc_cfg[k]) if norm_fn is None else norm_fn(num_channels=fc_cfg[k]),
...
def make_fc_layers_GN(fc_cfg, input_channels, output_channels=None, linear=True, norm_fn=None):
    fc_layers = []
    c_in = input_channels
    for k in range(0, fc_cfg.__len__()):
        fc_layers.extend([
            nn.Linear(c_in, fc_cfg[k], bias=False) if linear else nn.Conv1d(c_in, fc_cfg[k], kernel_size=1, bias=False),
            nn.BatchNorm1d(fc_cfg[k]) if norm_fn is None else norm_fn(fc_cfg[k]),
            # nn.GroupNorm(num_groups=fc_cfg[k]//8, num_channels=fc_cfg[k], eps=1e-3),
            nn.ReLU(inplace=True)
        ])
        c_in = fc_cfg[k]
    if output_channels is not None:
