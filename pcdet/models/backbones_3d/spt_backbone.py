...
from ...utils.spconv_utils import replace_feature, spconv, post_act_block, SparseBasicBlock, post_act_block_GN
...
class SSTInputLayer(nn.Module):
    ...
    def drop_single_shift(self, batch_win_inds):
        ...
        # But for all random runs, the voxels are all kept when I set a breakpoint and check.
    ...
    def drop_voxel(self, voxel_info, num_shifts):
        ...
        # a = drop_lvl_s0.shape[0]  # a: 7422

        drop_lvl_s0 = drop_lvl_s0[keep_mask_s0]
        # b = drop_lvl_s0.shape[0]  # a: 7422
        ...
        for k, v in voxel_info.items():
            if isinstance(v, torch.Tensor) and len(v) == voxel_num_before_drop:
                # batch_win_inds_shift0/1
                # coors_in_win_shift0/1
                # voxel_features
                # voxel_coords
                # voxel_shuffle_inds
                # voxel_keep_inds
                # voxel_drop_level_shift0/1
...
class ConvDown(nn.Module):
    def __init__(self, model_cfg, input_channels, indice_key, **kwargs):
        ...
        if stride > 1:
            # self.conv_down = post_act_block_GN(input_channels, d_model, 3, norm_fn=nn.GroupNorm(32, d_model, eps=1e-3),
            self.conv_down = post_act_block(input_channels, d_model, 3, norm_fn=norm_fn,
                                            stride=stride, padding=1,
                                            indice_key=f'{indice_key}_spconv', conv_type='spconv', dim=2)
        else:
            self.conv_down = None
...
class SSTBlockV1(nn.Module):
    def __init__(self, model_cfg, input_channels, indice_key, **kwargs):
        ...
        self.model_cfg = model_cfg
        encoder_cfg = model_cfg.ENCODER
        d_model_out = d_model = encoder_cfg.D_MODEL
        dim_feedforward = encoder_cfg.DIM_FEEDFORWARD
        self.half_channels = kwargs.get('half_channels', False)
        if self.half_channels:
            d_model = d_model // 2
            dim_feedforward = encoder_cfg.DIM_FEEDFORWARD // 2
        stride = encoder_cfg.STRIDE
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        if stride > 1:
            # self.conv_down = post_act_block_GN(input_channels, d_model, 3, norm_fn=nn.GroupNorm(32, d_model, eps=1e-3),
            self.conv_down = post_act_block(input_channels, d_model, 3, norm_fn=norm_fn,
                                               stride=stride, padding=1,
                                               indice_key=f'{indice_key}_spconv', conv_type='spconv', dim=2)
        elif self.half_channels:
            self.conv_down = post_act_block(input_channels, d_model, 1, norm_fn=norm_fn,
                                            indice_key=f'{indice_key}_spconv', conv_type='spconv', dim=2)
        else:
            self.conv_down = None
        ...
        for i in range(encoder_cfg.NUM_BLOCKS):
            block_list.append(
                BasicShiftBlockV2(d_model, encoder_cfg.NHEAD, dim_feedforward,
                                  encoder_cfg.DROPOUT, encoder_cfg.ACTIVATION, batch_first=False,
                                  layer_cfg=encoder_cfg.LAYER_CFG)
            )
        self.encoder_blocks = nn.ModuleList(block_list)
        # self.conv_out = post_act_block_GN(d_model, d_model, 3,
                                          # norm_fn=nn.GroupNorm(d_model//8, d_model, eps=1e-3),
                                          # indice_key=f'{indice_key}_subm', dim=2)
        self.conv_out = post_act_block(d_model, d_model_out, 3,
                                       norm_fn=norm_fn,
                                       indice_key=f'{indice_key}_subm', dim=2)
    ...
    def forward(self, sp_tensor):
        ...
        voxel_features_shuffle, voxel_coords_shuffle, voxel_shuffle_inds = self.encoder_forward(
            voxel_features, voxel_coords, grid_size)

        voxel_features_unshuffle = torch.zeros_like(voxel_features)
        voxel_features_unshuffle[voxel_shuffle_inds] = voxel_features_shuffle.to(dtype=voxel_features_unshuffle.dtype)
    ...
    def forward(self, batch_dict):
        ...
        for i, src in enumerate(self.model_cfg.FEATURES_SOURCE):
            ...
            spatial_features_stride.append(
                multi_scale_3d_strides[src] // self.model_cfg.FUSE_LAYER[src].UPSAMPLE_STRIDE)
        spatial_features = self.conv_out(torch.cat(spatial_features, dim=1))  # (B, C, Y, X)
        spatial_features_stride = spatial_features_stride[0]

        assert spatial_features.shape[0] == batch_size and spatial_features.shape[2] == self.grid_size[1] and \
               spatial_features.shape[3] == self.grid_size[0]
