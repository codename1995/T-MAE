...
from ...model_utils.network_utils import make_fc_layers, make_fc_layers_GN
from functools import partial
from ....utils import common_utils


class TemporalDynVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, grid_size, **kwargs):
        ...
        if mlps is not None:
            ...
            for i in range(len(mlps)):
                ...
                self.dvfe_mlps.append(make_fc_layers_GN(mlps[i], input_channels))
        ...
        self.finetuning = self.model_cfg.get('FT', False)
    ...
    def _forward(self, points):
        ...
        if self.finetuning:
            # reduce memory usage
            return 0, 0, 0, sampled_coords, x
        else:
            return points, coords, inverse_indices, sampled_coords, x
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        Returns:
            batch_dict:
        """
        points, coords, inverse_indices, sampled_coords, x = self._forward(batch_dict['points'])
        ...
        points, coords, inverse_indices, sampled_coords, x = self._forward(batch_dict['points_prev'])
        batch_dict['points_prev'] = points
        batch_dict['point_coords_prev'] = coords
        batch_dict['point_inverse_indices_prev'] = inverse_indices
        batch_dict['voxel_coords_prev'] = sampled_coords  # (M1+M2+..., 4)
        # batch_dict['pillar_features_prev'] = x
        batch_dict['voxel_features_prev'] = x  # (M1+M2+..., C)

        if self.finetuning:
            batch_dict.pop('points')
            batch_dict.pop('point_coords')
            batch_dict.pop('point_inverse_indices')
            batch_dict.pop('points_prev')
            batch_dict.pop('point_coords_prev')
            batch_dict.pop('point_inverse_indices_prev')
