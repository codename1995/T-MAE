...
def parse_config():
    ...
    parser.add_argument(
        "--wandb", action="store_true", default=False, help="whether to use wandb"
    )
    parser.add_argument(
        "--wandb_proj_name", type=str, default="t-mae-0.05", help="whether to use wandb"
    )
    parser.add_argument(
...
def main():
    ...
    if args.fixed_gap_eval >= 0:
        eval_output_dir = (
            output_dir / "eval" / ("eval_with_train_%d" % args.fixed_gap_eval)
        )
    else:
        eval_output_dir = output_dir / "eval" / "eval_with_train"
    eval_output_dir.mkdir(parents=True, exist_ok=True)
    args.start_epoch = max(
        args.epochs - args.num_epochs_to_eval + 1, 0
