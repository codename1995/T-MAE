...
def parse_config():
    ...
    parser.add_argument(
        "--max_waiting_mins", type=int, default=1, help="max waiting minutes"
    )
...
def eval_single_ckpt(
    ...
    # Print the difference between the model and the checkpoint
    logger.info("Comparing model architecture with checkpoint...")

    # Load checkpoint without applying to model
    ...
    # Compare shapes of matching keys
    for key in model_keys.intersection(ckpt_keys):
        if model_state_dict[key].shape != ckpt_state_dict[key].shape:
            logger.warning(
                f"Shape mismatch for key {key}: "
                f"model {model_state_dict[key].shape} vs "
                f"checkpoint {ckpt_state_dict[key].shape}"
            )

    logger.info("Finished comparing model architecture with checkpoint.")
