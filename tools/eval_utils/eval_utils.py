...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None, fuse_conv_bn=False):
    ...
    # Hard encode testing for inference speed
    eval_infer_speed = False
    ...
    if eval_infer_speed:
        # sh scripts/eccv_resub_final_0.05.sh fixed_infer
        # sh scripts/eccv_resub_final_0.05.sh fixed_infer
        ...
        print("Inference time: %f" % timings.mean())
        print("Inference time std" % timings.std())
        print("Inference time: %f" % timings[50:150].mean())
        print("Inference time std" % timings[50:150].std())
        pdb.set_trace()
